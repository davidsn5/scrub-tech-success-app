...
serve(async (req) => {
  ...
  try {
    ...
    if (user.email === "davidsonnf@example.com") {
    ...
    // Get subscriber info to check trial status
    ...
    if (subscriber && subscriber.status === "trial") {
      const trialEnd = new Date(subscriber.trial_end);
      const now = new Date();
      
      if (now < trialEnd) {
        logStep("User is in trial period", { trialEnd, now });
        return new Response(JSON.stringify({ 
          subscribed: true, 
        ...
        logStep("Trial expired", { trialEnd, now });
        return new Response(JSON.stringify({ 
          subscribed: false, 
